...
/**
 * See if we can tune VariGA parameters here
 * Created by Piers on 10/07/2015.
 */
public class Tuner {
    ...
    public static void main(String[] args) throws UnknownHostException {
        ...
        String[] levels = new String[]{"level1.txt", "level1E.txt", "level3.txt", "level4.txt", "level5.txt", "level6.txt"};
        ...
        for (int j = 0; j < gamesPerMatchup; j++) {
            for (int i = 0; i < games.length; i++) {
                ...
                while (ticksTaken < 2000 && !game.hasWon()) {
                    if(ticksTaken % 100 == 0) System.out.println("Tick: " + ticksTaken);
    ...
    public static Candidate mutate(Candidate p1) {
        ...
        for (int i = 0; i < child.chances.length; i++) {
            if (random.nextDouble() < 0.25) {
                ...
                if (child.chances[i] > 1.0) child.chances[i] = 1.0;
                if (child.chances[i] < 0.0) child.chances[i] = 0.0;
            }
        }

        if (random.nextDouble() < 0.25) {
            child.params[0] += random.nextBoolean() ? 1 : -1;
            if (child.params[0] == child.params[1]) child.params[1]++;
        }

        if (random.nextDouble() < 0.25) {
            child.params[1] += random.nextBoolean() ? 1 : -1;
            if (child.params[1] == child.params[0]) child.params[1]++;
        }

        if (random.nextDouble() < 0.25) {
            child.params[2] += random.nextBoolean() ? 1 : -1;
            if (child.params[2] == child.params[3]) child.params[3]++;
        }

        if (random.nextDouble() < 0.25) {
            child.params[3] += random.nextBoolean() ? 1 : -1;
            if (child.params[3] == child.params[2]) child.params[3]++;
        }
        return child;
    }
