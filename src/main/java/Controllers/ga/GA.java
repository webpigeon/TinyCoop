...
public class GA {
	...
	public ArrayList<MacroAction> m_actionList; // List of available actions to
												// govern the ship.
	...
	private GAIndividual breed() {
		...
		GAIndividual newBorn = gai1.uniformCross(gai2, m_rnd); // Cross and
																// return.
	...
	private GAIndividual getParent(GAIndividual first) {
		...
		while (tour[TOURNAMENT_SIZE - 1] == -1) {
			...
			boolean valid = m_individuals[part] != first; // Check it is not the
															// same selected
															// first.
			for (int k = 0; valid && k < i; ++k) {
				valid = (part != tour[k]); // Check it is not in the tournament
											// already.
			}
		...
		/*
		 * for(int j = 0; j < TOURNAMENT_SIZE; ++j) System.out.format(
		 * "%d (%.3f) ", tour[j], m_individuals[tour[j]].m_fitness);
		 * System.out.print(", ");
		 */
	...
	public void init(GameState a_gameState, boolean first) {
		...
		for (int i = 0; i < NUM_INDIVIDUALS; ++i) {
			...
			// System.out.format("individual i: " + i + ", fitness: %.3f,
			// actions: %s \n", m_individuals[i].m_fitness,
			// m_individuals[i].toString());
		}
	...
	public MacroAction run(GameState a_gameState, int generations, boolean first) {
		...
		while (currGen < generations) {
			...
			/*
			 * for(i = 0; i < m_individuals.length; ++i) System.out.format(
			 * "individual i: " + i + ", fitness: %.3f, actions: %s \n",
		...
		/*
		 * System.out.println(m_numGenerations); System.out.println("Best: " +
		 * m_individuals[0]); Game f =
		 * m_individuals[0].evaluate(a_gameState,m_gameEvaluator);
		 *
		 * System.out.println(" --------- Last generation " + m_numGenerations +
		 * " " + f.getWaypointsVisited() + "--------- "); for(int i = 0; i <
		 * m_individuals.length; ++i) System.out.format("individual i: " + i +
		 * ", fitness: %.3f, actions: %s \n", m_individuals[i].m_fitness,
		 * m_individuals[i].toString());
		 */
