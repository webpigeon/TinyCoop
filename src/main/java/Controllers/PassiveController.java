	...
	public Action get(GameState game) {
		...
		if (flare != null) {
			...
			if (!nextMoves.isEmpty()) {
				// System.out.println("I got an instruction and found a path:
				// "+nextMoves);
			} else {
				// System.out.println("I got an instruction and could not find a
				// path!");
			}
		}
			...
			for (Action action : gameState.getLegalActions(agentID)) {
				...
				if (action.isMovement()) {
					...
					if (actionResult.equals(nextMovePoint)) {
						// System.out.println("I am going to move now!
						// "+action);
						nextMove = action;
					}
				}
