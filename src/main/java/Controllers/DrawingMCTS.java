 ...
 */
public class DrawingMCTS extends Controller {
    ...
    private DrawingMCTSNode root;

    public DrawingMCTS(boolean first, int iterationLimit, int maxUCTDepth, int maxRolloutDepth) {
        this.first = first;
    ...
    public DrawingMCTS(boolean first, int iterationLimit){
        this.first = first;
        this.iterationLimit = iterationLimit;
    }
    ...
    public Action get(GameState game) {
        root = new DrawingMCTSNode(2.0, this, game.getLegalActions(first ? 0 : 1).length);
        DrawingMCTSNode travel;
...
//        System.out.println(root.getBestAction());
    ...
    public String getSimpleName() {
        return "DrawingMCTS: (" + iterationLimit + ";" + maxUCTDepth + ";" + maxRolloutDepth + ")";
    }
...
//            int radius = (int)( (entry.getValue() / max) * maxRadius);
    ...
    public Map<Point, Integer> calculateVisits(DrawingMCTSNode node, Point pos, Map<Point, Integer> visits){
        visits.put(pos, visits.getOrDefault(pos, 0) + node.getNumberOfVisits());
        if(node.getChildren() == null) return visits;
        for(DrawingMCTSNode child : node.getChildren()){
            if(child == null) continue;
//            if(child.getMoveToThisState().getY() == 0 && child.getMoveToThisState().getX() == 0) return visits;
    ...
    public Map<Point, Double> calculateScores(DrawingMCTSNode node, Point pos, Map<Point, Double> scores){
...
class DrawingMCTSNode {
    ...
    private DrawingMCTSNode parent;
    private DrawingMCTSNode[] children;
    private int childLength;

    private DrawingMCTS mcts;

    // Root
    public DrawingMCTSNode(double explorationConstant, DrawingMCTS mcts, int actionLength) {
    ...
    // Child
    public DrawingMCTSNode(DrawingMCTSNode parent, Action moveToThisState, int actionLength) {
        this.parent = parent;
    ...
    protected DrawingMCTSNode select(GameState state) {
        DrawingMCTSNode current = this;
        while (current.currentDepth < mcts.getMaxUCTDepth() && !state.hasWon()) {
            DrawingMCTSNode expandedChild = current.expand(state);
...
//            if (current.isFullyExpanded(state)) {
    ...
    protected DrawingMCTSNode selectBestChild() {
        int selected = -1;
    ...
    private DrawingMCTSNode expand(GameState state) {
        int bestAction = -1;
        double bestValue = -Double.MAX_VALUE;
        if (children == null) children = new DrawingMCTSNode[childLength];
        ...
        children[bestAction] = new DrawingMCTSNode(this, allActions[bestAction], state.getActionLength());
    ...
    public DrawingMCTSNode getParent() {
        return parent;
    }

    public DrawingMCTSNode[] getChildren() {
        return children;
    }
