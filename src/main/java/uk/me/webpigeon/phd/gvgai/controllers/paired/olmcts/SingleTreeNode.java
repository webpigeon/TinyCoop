package uk.me.webpigeon.phd.gvgai.controllers.paired.olmcts;
...
public class SingleTreeNode
{
    ...
    public SingleTreeNode(SingleTreeNode parent, int childIdx, Random rnd) {
        ...
        if(parent != null)
            ...
            m_depth = 0;
        children = new SingleTreeNode[MirrorAgent.NUM_ACTIONS[MirrorAgent.id]];
    }
    ...
    public void mctsSearch(ElapsedCpuTimer elapsedTimer) {
        ...
        while(remaining > avgTimeTaken && remaining > remainingLimit){
    ...
    public SingleTreeNode expand(StateObservationMulti state) {
        ...
        Action[] acts = new Action[MirrorAgent.no_players];

        //set this agent's action
        acts[MirrorAgent.id] = MirrorAgent.actions[MirrorAgent.id][bestAction];

        //get actions available to the opponent and assume they will do a random action
        Action[] oppActions = MirrorAgent.actions[MirrorAgent.oppID];
        acts[MirrorAgent.oppID] = oppActions[new Random().nextInt(oppActions.length)];
    ...
    public SingleTreeNode uct(StateObservationMulti state) {
        ...
        //set this agent's action
        acts[MirrorAgent.id] = MirrorAgent.actions[MirrorAgent.id][selected.childIdx];
    ...
    public double rollOut(StateObservationMulti state)
    {
        ...
        while (!finishRollout(state,thisDepth)) {
        	...
        	Action[] acts = new Action[MirrorAgent.no_players];
            for (int i = 0; i < MirrorAgent.no_players; i++) {
                acts[i] = MirrorAgent.actions[i][m_rnd.nextInt(MirrorAgent.NUM_ACTIONS[MirrorAgent.id])];
            }
            state.advance(acts);
            thisDepth++;
        }
    ...
    public double value(StateObservationMulti a_gameState) {
        ...
        int win = a_gameState.getMultiGameWinner()[MirrorAgent.id];
        double rawScore = a_gameState.getGameScore(MirrorAgent.id);
