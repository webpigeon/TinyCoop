...
public class GVGAIMadeMeRewriteMyRunner {
	...
	public static void main(String[] args) throws Exception {
		if (args.length != 1) {
			System.err.println("usage: [agentname]");
			System.exit(-1);
		}
			...
			for (String level : levels) {
				...
				levelRel.setLegalMoves("relative", Filters.getBasicActionsWithComms());
					...
					for (String agent2 : player2List) {
						...
						Controller predictorAgentP2 = controllers.parseCoopDescription(GameState.PLAYER_1, game, agent2);
						...
						Controller predictorAgentP1 = controllers.parseCoopDescription(GameState.PLAYER_0, game, agent2);
				...
				// write each set of data at after each run
				writeResults(results, runid.toString());
			}
		}

		// writeResults(results, UUID.randomUUID().toString());
		System.out.println(runid + " done.");
	}
	...
	private static void writeResults(List<GameResult> results, String runID) {
		...
		try (OutputStream out = new FileOutputStream(String.format("results-%s.csv", runID), true);) {
			GenerateCSV csv = new GenerateCSV(out);
			csv.writeLine("gameID", "levelID", "actionSet", "player1", "player2", "result", "score", "ticks",
					"wallTime", "userTime", "runID");

			for (GameResult result : results) {

				// detect timeouts and respond correctly
				if (result != null) {
					csv.writeLine(result.setup.id, result.setup.level, result.setup.actionSet, result.setup.p1,
							result.setup.p2, result.result, result.score, result.ticks, result.wallTime, result.cpuTime,
							runID);
				}
