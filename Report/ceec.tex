...
\includegraphics[scale=1]{level5.txt-pairs-ticks}
...
\caption{Graph showing the average score obtained by each \emph{Agent} across level 1(Blue) and level 1E (Orange)}
\label{1v1eS}
\end{figure}
...
\caption{Graph showing the average ticks taken for each \gls{ai} \emph{Agent} across level 1(Blue) and level 1E (Orange)}
\label{1v1eT}
\end{figure}

Figure \ref{1v1eS} shows the effect that having the complex \emph{Buttons} and \emph{Doors} in a level has on the score an Agent achieves. Figure \ref{1v1eT} demonstrates the vast improvement in path finding ability to a goal in the VariGA compared to the GAController. This is expected due to the ability to make individual actions as compared to the GAController which is only able to make 3 identical turns at a time leading to convoluted paths compared to optimum.
...
\begin{figure}[ht]
\centering
\includegraphics[ width = \linewidth]{avgScoreOverMap5}
...
There is scope for more work in a number of areas. More work can be conducted in the writing of AI's to use communication in order to complete these challenges better amongst themselves. A framework for communication would be devised - with the requirement for fairly \gls{ggp} restrictions in mind.
